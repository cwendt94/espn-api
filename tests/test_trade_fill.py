...
def _card_trade(txn_id='card-1', related='rel-1', player_id=1001, week=6, extra_items=None):
    items = [{
        'type': 'TRADE',
        'playerId': player_id,
        'fromTeamId': 2,
...
class TradeFillHelperTest(TestCase):
    def test_prefers_player_with_more_trade_legs(self):
        short = Player(_card_player([_card_trade(txn_id='a', related='rel-1', player_id=1)]), 2022)
        long = Player(_card_player([_card_trade(
            txn_id='b',
            related='rel-1',
            player_id=1,
            extra_items=[{'type': 'TRADE', 'playerId': 2, 'fromTeamId': 3, 'toTeamId': 2}],
        )]), 2022)
    ...
    def test_fills_empty_weekly_row_from_player_transactions(self):
        weekly = [_empty_trade()]
        player = Player(_card_player([_card_trade()]), 2022, player_map={1001: 'Justin Jefferson'})
