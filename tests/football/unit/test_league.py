...
class LeagueTest(TestCase):
    ...
    def test_load_roster_week(self, m):
        ...
        m.get(
            self.espn_endpoint + "?view=mRoster&scoringPeriodId=1",
            status_code=200,
            json=data,
        )
    ...
    def test_get_scoreboard(self, m):
        ...
        self.assertEqual(
            repr(scoreboard[1]),
            "Matchup(Team(Watch What  You Saquon), Team(Feel the  Brees))",
        )
        ...
        self.assertEqual(
            repr(scoreboard[-1]),
            "Matchup(Team(Jacking Goff  On Sundays), Team(Feel the  Brees))",
        )
    ...
    def test_draft(self, m):
        ...
        self.assertEqual(
            repr(first_pick), "Pick(R:1 P:1, Le'Veon Bell, Team(Rollin' With Mahomies))"
        )
        self.assertEqual(third_pick.round_num, 1)
        self.assertEqual(third_pick.round_pick, 3)
        self.assertEqual(
            third_pick.auction_repr(),
            "Team(Goin' HAM Newton), 13934, Antonio Brown, 0, False",
        )
    ...
    def test_free_agents(self, m, mock_boxplayer, mock_nfl_schedule, mock_pos_ratings):
        ...
        league = League(self.league_id, self.season)
        m.get(
            self.espn_endpoint + "?view=kona_player_info&scoringPeriodId=16",
            status_code=200,
            json={"players": [1, 2]},
        )
        league.year = 2019
    ...
    def test_recent_activity(self, m):
        ...
        m.get(
            self.espn_endpoint + "?view=kona_playercard",
            status_code=200,
