...
class OfferTest(TestCase):
    def _make_offer(self, status='EXECUTED', bid_amount=20, player_id=1001, team_id=3,
                    process_date=1700000000000, add_item=True, drop_player_id=999):
        data = {
            'status': status,
            'id': 42,
    ...
    def test_processed_offer_sets_expected_attributes(self):
        offer = self._make_offer(status='EXECUTED', bid_amount=25, player_id=1001, team_id=7)
    ...
    def test_offer_ordering_uses_status_then_bid_amount(self):
        ...
        self.assertEqual([offer.result for offer in ordered], ['Canceled', 'Outbid', 'Processed'])
