...
class PlayerStatSplitsTest(TestCase):
    ...
    def test_season_split_populates_stats(self):
        ...
        self.assertIn(
            0, player.stats["season"] if "season" in player.stats else player.stats
        )
    ...
    def test_box_score_split_in_stats_dict(self):
        ...
        player = self._player_with_stats(
            [_make_stat(5, scoring_period=3, applied_total=7.0)]
        )
    ...
    def test_wrong_season_ignored(self):
        player = self._player_with_stats(
            [_make_stat(0, season_id=2020, applied_total=99.0)]
        )
    ...
    def test_projected_split(self):
        player = self._player_with_stats(
            [_make_stat(0, stat_source_id=1, applied_total=25.0)]
        )
        self.assertIn(0, player.stats_splits["season"])
    ...
    def test_projected_total_points_from_projected_split(self):
        player = self._player_with_stats(
            [_make_stat(0, stat_source_id=1, applied_total=38.5)]
        )
    ...
    def test_batter_stats_filtered_for_pitchers(self):
        ...
        pitcher_data = _make_player_data(
            default_position_id=1,
            eligible_slots=[14],
            player_extras={"stats": [_make_stat(0, stats={"5": 2.0, "48": 10.0})]},
        )
    ...
    def test_pitcher_stats_filtered_for_batters(self):
        ...
        batter_data = _make_player_data(
            default_position_id=2,
            eligible_slots=[0],
            player_extras={"stats": [_make_stat(0, stats={"5": 2.0, "47": 3.45})]},
        )
