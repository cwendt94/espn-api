...
class Matchup(object):
    ...
    def _fetch_matchup_info(self, data):
        ...
        if (
            ...
            self.home_team_cats = {
                STATS_MAP[i]: {
                    "score": data["home"]["cumulativeScore"]["scoreByStat"][i]["score"],
                    "result": data["home"]["cumulativeScore"]["scoreByStat"][i][
                        "result"
                    ],
                }
                for i in data["home"]["cumulativeScore"]["scoreByStat"].keys()
            }

            self.away_team_cats = {
                STATS_MAP[i]: {
                    "score": data["away"]["cumulativeScore"]["scoreByStat"][i]["score"],
                    "result": data["away"]["cumulativeScore"]["scoreByStat"][i][
                        "result"
                    ],
                }
                for i in data["away"]["cumulativeScore"]["scoreByStat"].keys()
            }
