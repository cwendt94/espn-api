...
class Matchup(object):
    ...
    def __init__(self, data):
        ...
        (
            self.home_team,
            self.home_final_score,
            self.home_team_cats,
            self.home_team_live_score,
        ) = self._fetch_matchup_info(data, "home")
        (
            self.away_team,
            self.away_final_score,
            self.away_team_cats,
            self.away_team_live_score,
        ) = self._fetch_matchup_info(data, "away")
    ...
    def _fetch_matchup_info(self, data, team):
        ...
        if (
            "cumulativeScore" in data[team].keys()
            and data[team]["cumulativeScore"]["scoreByStat"]
        ):

            team_live_score = (
                data[team]["cumulativeScore"]["wins"]
                + data[team]["cumulativeScore"]["ties"] / 2
            )

            team_cats = {
                STATS_MAP.get(i, i): {
                    "score": data[team]["cumulativeScore"]["scoreByStat"][i]["score"],
                    "result": data[team]["cumulativeScore"]["scoreByStat"][i]["result"],
                }
                for i in data[team]["cumulativeScore"]["scoreByStat"].keys()
            }
