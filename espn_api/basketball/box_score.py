...
class BoxScore(ABC):
    ...
    def _get_player_lineup(self, team, data, pro_schedule, by_matchup, year):
        ...
        roster_key = (
            "rosterForMatchupPeriod" if by_matchup else "rosterForCurrentScoringPeriod"
        )
        roster = data[team].get(roster_key, {})
        lineup = [
            BoxPlayer(player, pro_schedule, year, self.scoring_period)
...
class H2HPointsBoxScore(BoxScore):
    def __init__(self, data, pro_schedule, by_matchup, year, scoring_period=0):
        ...
        self.home_score, self.home_projected, self.home_lineup = self._get_team_data(
            "home", data, pro_schedule, by_matchup, year
        )

        self.away_score, self.away_projected, self.away_lineup = self._get_team_data(
            "away", data, pro_schedule, by_matchup, year
        )
...
class H2HCategoryBoxScore(BoxScore):
    def __init__(self, data, pro_schedule, by_matchup, year, scoring_period=0):
        ...
        (
            ...
            self.home_stats,
            self.home_lineup,
        ) = self._get_team_data("home", data, pro_schedule, by_matchup, year)
        ...
        (
            ...
            self.away_stats,
            self.away_lineup,
        ) = self._get_team_data("away", data, pro_schedule, by_matchup, year)
...
get_box_scoring_type_class = lambda scoring_type: ScoringType.get(
    scoring_type, H2HPointsBoxScore
)
