...
class BoxScore(object):
    ...
    def __init__(self, data, pro_schedule, positional_rankings, week, year):
        ...
        if "totalPointsLive" in data["home"]:
            ...
            self.home_score = round(
                data["home"]["rosterForCurrentScoringPeriod"]["appliedStatTotal"], 2
            )
        home_roster = data["home"]["rosterForCurrentScoringPeriod"]["entries"]
        self.home_lineup = [
            BoxPlayer(player, pro_schedule, positional_rankings, week, year)
        ...
        if "away" in data:
            ...
            if "totalPointsLive" in data["away"]:
                ...
                self.away_score = round(
                    data["away"]["rosterForCurrentScoringPeriod"]["appliedStatTotal"], 2
                )
