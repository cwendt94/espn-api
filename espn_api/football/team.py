...
class Team(object):
    ...
    def __init__(self, data, roster, schedule, year, **kwargs):
        ...
        self.acquisition_budget_spent = data.get("transactionCounter", {}).get(
        ...
        self.playoff_pct = (
            data.get("currentSimulationResults", {}).get("playoffPct", 0) * 100
        )
        ...
        self.stats = {
            PLAYER_STATS_MAP.get(int(i), i): j
            for i, j in data.get("valuesByStat", {}).items()
        }
    ...
    def _fetch_schedule(self, data):
        ...
        for matchup in data:
            ...
            if self.team_id in (home_id, away_id):
                ...
                current_team, opponent_id, away = (
                    (home_team, away_id, False)
                    if home_id == self.team_id
                    else (away_team, home_id, True)
                )
