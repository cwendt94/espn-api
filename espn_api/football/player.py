...
class Player(object):
    ...
    def __init__(
        ...
        for stats in player_stats:
            ...
            breakdown = {
                PLAYER_STATS_MAP.get(int(k), k): v for (k, v) in stats_breakdown.items()
            }
            # fantasy stats (points per td, ppr, points per yard bucket)
            applied_stats = stats.get("appliedStats", {})
            points_breakdown = {
                PLAYER_STATS_MAP.get(int(k), k): v for (k, v) in applied_stats.items()
            }
            ...
            points_type, breakdown_type, points_breakdown_type, avg_type = (
                ("points", "breakdown", "points_breakdown", "avg_points")
                if stat_source == 0
                else (
                    "projected_points",
                    "projected_breakdown",
                    "projected_points_breakdown",
            ...
            if self.stats.get(scoring_period):
                ...
                self.stats[scoring_period] = {
                    ...
                    points_breakdown_type: points_breakdown,
                    avg_type: avg_points,
                }
            if not stat_source:
