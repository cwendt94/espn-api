from espn_api.hockey.box_player import BoxPlayer
...
class BoxScore(object):
    ...
    def __init__(self, data, pro_schedule, by_matchup):
        ...
        if "totalPointsLive" in data["home"]:
            ...
            self.home_projected = round(
                data["home"].get("totalProjectedPointsLive", -1), 2
            )
        else:
            self.home_score = round(home_roster.get("appliedStatTotal", 0), 2)
        self.home_lineup = [
            BoxPlayer(player, pro_schedule) for player in home_roster.get("entries", [])
        ]
        ...
        if "away" in data:
            ...
            if "totalPointsLive" in data["away"]:
                ...
                self.away_projected = round(
                    data["away"].get("totalProjectedPointsLive", -1), 2
                )
            else:
                self.away_score = round(away_roster.get("appliedStatTotal", 0), 2)
            self.away_lineup = [
                BoxPlayer(player, pro_schedule)
                for player in away_roster.get("entries", [])
            ]
