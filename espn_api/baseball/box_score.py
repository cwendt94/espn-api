...
class H2HCategoryBoxScore(BoxScore):
    ...
    def _process_team(self, team_data, is_home_team):
        ...
        if team_data is not None:
            ...
            team["stats"] = {}
            for stat_key, stat_dict in team_data["cumulativeScore"][
                "scoreByStat"
            ].items():
...
class H2HPointsBoxScore(BoxScore):
    ...
    def __init__(self, data, pro_schedule, year, scoring_period=0):
        ...
        self.home_team, self.home_score, self.home_projected, self.home_lineup = (
            self._get_team_data("home", data, pro_schedule, scoring_period, year)
        )

        self.away_team, self.away_score, self.away_projected, self.away_lineup = (
            self._get_team_data("away", data, pro_schedule, scoring_period, year)
        )
    ...
    def _get_team_data(self, team, data, pro_schedule, week, year):
        if team not in data:
            return (
                ...
                -1,
                [],
            )  # -1 projected score indicates no projection available (bye week / missing)
        ...
        team_roster = (
            data[team].get("rosterForCurrentScoringPeriod", {}).get("entries", [])
        )
        team_lineup = [
            BoxPlayer(player, pro_schedule, week, year) for player in team_roster
        ]
...
class RotoBoxScore(BoxScore):
    ...
    def __init__(self, data, pro_schedule, year, scoring_period=0):
        ...
        for team_data in data.get("teams", []):
            ...
            total = round(
                live if live is not None else team_data.get("totalPoints", 0), 2
            )

            stats = {}
            for stat_id_str, stat_dict in (
                team_data.get("cumulativeScore", {}).get("scoreByStat", {}).items()
            ):
            ...
            entries = team_data.get("rosterForCurrentScoringPeriod", {}).get(
                "entries", []
            )
