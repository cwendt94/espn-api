...
class Player(object):
    ...
    def __init__(self, data, year, player_map=None, get_team_data=None):
        self.name = json_parsing(data, "fullName")
        self.playerId = json_parsing(data, "id")
        self.position = DEFAULT_POSITION_MAP.get(
            json_parsing(data, "defaultPositionId"),
            str(json_parsing(data, "defaultPositionId")),
        )
        ...
        self.acquisitionDate = (
            datetime.fromtimestamp(raw_acq_date / 1000) if raw_acq_date else None
        )  # ESPN timestamps are milliseconds
        ...
        self.last_news_date = (
            datetime.fromtimestamp(raw_news_date / 1000) if raw_news_date else None
        )  # ESPN timestamps are milliseconds
        ...
        self.draft_ranks = {
            rank_type: {
                "rank": rank_data["rank"],
                "auction_value": rank_data["auctionValue"],
            }
        ...
        for stats in player_stats:
            ...
            for k, v in stats_breakdown.items():
                stat_id = int(k)
                if not is_eligible_pitcher and stat_id in PITCHER_ONLY_STATS:
                    continue  # skip pitcher-only stats if not eligible for pitcher positions
                if not is_eligible_batter and stat_id in BATTER_ONLY_STATS:
                    continue  # skip batter-only stats if not eligible for batter positions
                filtered_breakdown[k] = v
            breakdown = {
                STATS_MAP.get(int(k), k): v for (k, v) in filtered_breakdown.items()
            }
