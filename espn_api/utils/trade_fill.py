...
def player_transactions(data, transaction_class, player_map=None, get_team_data=None):
    ...
    return [transaction_class(raw, names, team_fn) for raw in player_card_transactions(data)]


def transaction_trade_legs(txn):
    return [item for item in (getattr(txn, 'items', None) or []) if getattr(item, 'type', None) == 'TRADE']


def transaction_related_key(txn):
    return str(getattr(txn, 'related_transaction_id', None) or getattr(txn, 'id', None) or '')
...
def fill_trade_accept_from_players(transactions, players, scoring_period=None):
    ...
    for txn in transactions:
        ...
        if not getattr(txn, 'related_transaction_id', None) and getattr(card, 'related_transaction_id', None):
    ...
    for key, card in best.items():
        ...
        if key in seen or card_id in seen:
            continue
        if scoring_period is not None and getattr(card, 'scoring_period', None) != scoring_period:
            continue
