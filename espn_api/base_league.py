...
from .utils.trade_fill import PLAYER_CARD_BATCH, chunked, fill_trade_accept_from_players, transaction_trade_legs
...
class BaseLeague(ABC):
    ...
    def _scoring_period_roster_ids(self, scoring_period):
        ...
        data = self.espn_request.league_get(params={
            'view': 'mRoster',
    ...
    def _trade_fill_player_ids(self, scoring_period, player_ids=None, fill_from='week'):
        if player_ids is not None:
            return {player_id for player_id in player_ids if isinstance(player_id, int) and player_id != 0}
    ...
    def _fill_trade_accept_from_player_cards(
        ...
        empty_trades = [
            ...
            if getattr(txn, 'type', None) == 'TRADE_ACCEPT' and not transaction_trade_legs(txn)
        ]
        if not empty_trades:
            return transactions
        ...
        for batch in chunked(sorted(ids), PLAYER_CARD_BATCH):
            data = self.espn_request.get_player_card(batch, getattr(self, 'finalScoringPeriod', scoring_period))
